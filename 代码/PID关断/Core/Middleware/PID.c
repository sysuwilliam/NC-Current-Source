...
{
    static uint32_t stable_Current_counter = 0;
    static float DAC2_stable = 0.0f;
    static float DAC2_sum = 0.0f;
    float err_stable = I_set * 0.005f;


    if (Current_PID==PID_Idle) {
        stable_Current_counter = 0;
        DAC2_sum = 0.0f;
        return;
    }
    ...
    /* ======= 3. 工业级死区控制（防微幅震荡干扰） ======= */
    int32_t dynamic_stable_th = (int32_t)(I_set * 0.005f * PID_CURRENT_SCALE);
    if (dynamic_stable_th < 2) {
        dynamic_stable_th = 2;
    }

    int32_t current_deadband = dynamic_stable_th / 2;
    if (current_deadband < 1) {
        current_deadband = 1;
    }

    // 注意：这里求最小误差时，必须用“拦截死区前”的原始 err_curr，否则全是 0 就无法区分谁最优了
    int32_t raw_abs_err = abs(pid_current.err_curr);

    if (abs(pid_current.err_curr) < current_deadband)
    {
        pid_current.err_curr = 0;
    }

    if (raw_abs_err < dynamic_stable_th)
    {
        stable_Current_counter++;

        DAC2_sum += DAC2_cmd;

        if (stable_Current_counter >= PID_CURRENT_Stable_time) {
            Current_PID = PID_Idle;
            DAC2_stable = DAC2_sum / (float)stable_Current_counter;

            DAC_Set_Voltage(DAC_CHANNEL_2, DAC_GAIN_1X, DAC2_stable);
            stable_Current_counter = 0;
            DAC2_sum = 0.0f;
            return;
        }
...
void BUCK_Loop(void) {
    ...
    if (HAL_GPIO_ReadPin(BUCK_EN_GPIO_Port, BUCK_EN_Pin) == GPIO_PIN_RESET) {
        DAC_Set_Voltage(DAC_CHANNEL_1, DAC_GAIN_2X, 0.0f);
        return;
    }
