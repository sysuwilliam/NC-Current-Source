...
void Filter_Output(void) {
    ...
    /* ⚡【工业防线】：由于铲除了上方的中途退出，只要均值滤波窗口积满（NA次），
     * 这一行在任何物理情况下都必定会被 100% 执行。标志位逻辑无懈可击。 */
    ADC_FLAG = 1;
}
...
void PID_Init(void)
{
    ...
    pid_buck.Kp = 0.0007f;
    pid_buck.Ki = 0.00025f;
    pid_buck.Kd = 0.0f;

    pid_buck.target = (int32_t)(1.2f * PID_VOUTP_SCALE);
    pid_buck.actual = 0;

    pid_buck.err_curr = 0;
    pid_buck.err_last1 = 0;
    pid_buck.err_last2 = 0;
    pid_buck.out_delta = 0.0f;
}
...
void PID_BUCK_Loop(void) {
    ...
    pid_buck.actual = (int32_t)(Vmos * PID_VOUTP_SCALE);

    // 2. 计算当前误差 E(n)
    pid_buck.err_curr = pid_buck.target - pid_buck.actual;

    /* ======= 3. 工业级死区控制（防微幅震荡干扰） ======= */
    // 使用死区宏进行边界拦截，低于死区阈值则判定误差为0
    if (abs(pid_buck.err_curr) < PID_VOUTP_DEADBAND)
    {
        pid_buck.err_curr = 0;
    }

    /* ======= 4. 标准增量式 PID  ======= */
    pid_buck.out_delta = (pid_buck.Kp * (float)(pid_buck.err_curr - pid_buck.err_last1)) +
                         (pid_buck.Ki * (float)pid_buck.err_curr) +
                         (pid_buck.Kd * (float)(pid_buck.err_curr - 2 * pid_buck.err_last1 + pid_buck.err_last2));

    /* ======= 5. 作用于外设：改变 DAC2 目标控制量 ======= */
    DAC1_cmd += pid_buck.out_delta;
    ...
    // 8. 滚动的状态历史记录前移
    pid_buck.err_last2 = pid_buck.err_last1;
    pid_buck.err_last1 = pid_buck.err_curr;
}
...
void BUCK_Loop(void) {
    ...
    /* =================================================================
     * ⚡ 4. 核心纠偏：全回路阻抗前馈模型 (彻底斩断与 VOUTN 的动态正反馈)
     * =================================================================
     * 理想公式：VOUT+ = 整个回路的总压降
     * = 负载压降 + 采样电阻压降 + MOS管维持恒流所需的饱和预留余量
     * = I_set * Rload_disp + I_set * Rs + Vmos_target
     * * 习惯法遵循：必须显式加上 MOS 管 1.5V 的工作余量。如果不加，留给 MOS 的电压为 0V，
     * 后级恒流 PID 就会因没有压降驱动而瞬间陷入恒流失效（拉不出力气）。
     */
#define VMOS_STATIC_TARGET    1.5f   // 强制物理留空防线：给后级 MOS 管预留稳定的 1.5V 管压降
#define RSENSE_VALUE          2.0f   // 你的采样电阻实际阻值 (Ω)，请根据硬件实际值修改（如 1.0f 或 2.0f）

    // 💥 纯前馈算力核心：整个公式里没有任何“会随 DAC1 改变而改变”的反馈变量，从根本上杜绝一直上升！
    float VOUTP_target = (I_set * Rload_disp) + (I_set * RSENSE_VALUE) + VMOS_STATIC_TARGET;
